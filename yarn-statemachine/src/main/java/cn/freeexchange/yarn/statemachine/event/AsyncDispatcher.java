...
package cn.freeexchange.yarn.statemachine.event;
...
import cn.freeexchange.yarn.statemachine.service.AbstractService;
...
public class AsyncDispatcher extends AbstractService implements Dispatcher {
	...
	protected void serviceInit(Configuration conf) throws Exception {
		this.exitOnDispatchException = Dispatcher.DEFAULT_DISPATCHER_EXIT_ON_ERROR;
	...
	protected void serviceStop() throws Exception {
		if (drainEventsOnStop) {
			...
			getConfig();
			long endTime = System.currentTimeMillis() + Configuration.DEFAULT_DISPATCHER_DRAIN_EVENTS_TIMEOUT;
			...
			synchronized (waitForDrained) {
				while (!drained && eventHandlingThread != null && eventHandlingThread.isAlive()
					...
					LOG.info(
							"Waiting for AsyncDispatcher to drain. Thread state is :" + eventHandlingThread.getState());
				}
			}
		}
		stopped = true;
