...
/**
 * This class contains a set of methods to work with services, especially to
 * walk them through their lifecycle.
 */
public final class ServiceOperations {
	private static Logger LOG = LoggerFactory.getLogger(ServiceOperations.class);
	...
	public static class ServiceListeners {
		/**
		 * List of state change listeners; it is final to guarantee that it will
		 * never be null.
		 */
		private final List<ServiceStateChangeListener> listeners = new ArrayList<ServiceStateChangeListener>();

		/**
		 * Thread-safe addition of a new listener to the end of a list. Attempts
		 * to re-register a listener that is already registered will be ignored.
		 * 
		 * @param l
		 *            listener
		 */
		...
		/**
		 * Change to a new state and notify all listeners. This method will
		 * block until all notifications have been issued. It caches the list of
		 * listeners before the notification begins, so additions or removal of
		 * listeners will not be visible.
		 * 
		 * @param service
		 *            the service that has changed state
		 */
