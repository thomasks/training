...
public class CompositeService extends AbstractService {
	...
	private static Logger LOG = LoggerFactory.getLogger(CompositeService.class);

	/**
	 * Policy on shutdown: attempt to close everything (purest) or only try to
	 * close started services (which assumes that the service implementations
	 * may not handle the stop() operation except when started. Irrespective of
	 * this policy, if a child service fails during its init() or start()
	 * operations, it will have stop() called on it.
	 */
	 ...
	 * 
	 * @return a list of child services at the time of invocation - added
	 *         services will not be picked up.
	 */
	...
	/**
	 * If the passed object is an instance of {@link Service}, add it to the
	 * list of services managed by this {@link CompositeService}
	 * 
	 * @param object
	 ...
	 *            index from where the stop should work
	 * @param stopOnlyStartedServices
	 *            flag to say "only start services that are started, not those
	 *            that are NOTINITED or INITED.
	 * @throws RuntimeException
	 *             the first exception raised during the stop process -<i>after
	 *             all services are stopped</i>
	 */
